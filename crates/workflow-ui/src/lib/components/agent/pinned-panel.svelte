	...
	<div class="flex-1 flex items-center justify-center">
		<div class="flex flex-col items-center gap-2 text-center px-4">
			<div class="size-10 rounded-full bg-muted/50 flex items-center justify-center">
				<Pin class="size-4 text-muted-foreground/40" />
			</div>
			<p class="text-xs text-muted-foreground/60">No pinned messages</p>
			<p class="text-[11px] text-muted-foreground/40">Click the pin icon on any message to save it here for quick access.</p>
		</div>
	</div>
{:else}
	<ScrollArea class="flex-1 min-h-0 no-scrollbar">
		<div class="py-2 px-3 flex flex-col gap-2">
			{#each messages as pin (pin.id)}
				<div class="group relative rounded-lg border border-border/40 bg-background/50 p-3">
					<div class="flex items-center justify-between mb-2">
						<div class="text-[10px] font-medium text-muted-foreground/40 uppercase tracking-wider flex items-center gap-2">
							<span>{pin.type}</span>
							{#if pin.agentRole}
					...
					<div class="text-xs leading-relaxed">
						{#if pin.type === "user" || pin.type === "assistant"}
							<div class="prose-sm dark:prose-invert prose-code:before:content-none prose-code:after:content-none max-w-full">
						...
						{:else if pin.type === "tool"}
							<ToolCard name={pin.text} result={pin.result ?? undefined} status={pin.status ?? "done"} />
