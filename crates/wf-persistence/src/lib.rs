...
use wf_models::models::{CustomProvider, ModelRegistry};
...
pub fn save_session(messages: &[wf_core::ChatMessage]) -> Result<()> {
...
pub fn load_session() -> Option<Vec<wf_core::ChatMessage>> {
...
pub fn save_session_as(name: &str, messages: &[wf_core::ChatMessage]) -> Result<()> {
...
pub fn load_session_as(name: &str) -> Option<Vec<wf_core::ChatMessage>> {
    let path = config_dir()
        .ok()?
        .join("sessions")
