...
macro_rules! build_chat_agent {
    ($client:expr, $model:expr, $system:expr, $params:expr) => {{
        let mut builder = $client
            .agent($model)
            .preamble($system)
            .temperature(wf_core::DEFAULT_TEMPERATURE)
            .max_tokens(wf_core::DEFAULT_MAX_TOKENS);
    ...
    ($client:expr, $model:expr, $system:expr, $handle:expr, $msg:expr, $history:expr, $params:expr) => {{
        ...
        Ok(Self::wrap_tool_stream(
            agent
                ...
                .multi_turn(wf_core::DEFAULT_MAX_TOOL_TURNS)
                .await,
        ))
    }};
}
                    ...
                    Ok(MultiTurnStreamItem::StreamAssistantItem(
                        ...
                        if total_tool_calls > wf_core::MAX_TOOL_CALLS_PER_STREAM {
                            ...
                            tracing::warn!(
                                ...
                                total_tool_calls,
                                wf_core::MAX_TOOL_CALLS_PER_STREAM,
                                summary,
                            );
                        ...
                        *per_tool_count.entry(tool_name.clone()).or_insert(0) += 1;
                        if next_count > wf_core::MAX_CALLS_PER_TOOL {
                            tracing::warn!(
                                "Tool loop closed: '{}' called {} times (limit {}): {}",
                                tool_name,
                                next_count,
                                wf_core::MAX_CALLS_PER_TOOL,
                                summary,
                            );
                            yield ToolEvent::Text(format!(
