...
mod tests {
    ...
    fn try_embedding() -> Option<wf_llm::embedding::EmbeddingService> {
        std::panic::catch_unwind(wf_llm::embedding::EmbeddingService::new).ok()
    }
    ...
    async fn test_check_rules_with_embedding() {
        ...
        let ctx = RuleContext {
            ...
            tool_trace: "",
            embedding: Some(&emb as &dyn wf_llm::EmbeddingService),
            cfg: Some(&cfg),
        };
        let report = check_rules(&cfg, &registry, &ctx).await;
        assert!(report.all_passed);
