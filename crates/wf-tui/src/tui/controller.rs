...
//! moved to [`crate::effect`].  This module only contains
...
pub fn ensure_initial_agent_sync(core: &mut CoreState, goal_hint: &str) -> Option<AgentId> {
        ...
        Ok(rt) => match core.agent_pool.try_write() {
            Ok(mut pool) => {
                ...
                let existing = pool
                    ...
                    .iter()
                    .find(|a| {
                        a.role == core.default_role && a.status == wf_agent::AgentStatus::Idle
                    })
                    .map(|a| a.id);
    ...
    if let Some(agent_id) = agent_id {
        ...
        let entry = AgentEntry {
            id: wf_agent::AgentPool::agent_id_str(&agent_id),
            name: format!(
                "planner-{:04x}",
...
pub fn switch_session(core: &mut CoreState, ui: &mut UiState, name: &str) {
    let Some(messages) = wf_persistence::load_session_as(name) else {
        return;
    };
    ...
    if let Some((prompt, role)) = wf_persistence::load_session_prompt(name) {
...
pub fn get_or_create_provider_client(
    ...
    if is_custom_provider(provider_id) && !state.api_keys.contains_key(&env_key) {
        let llm_provider = LlmProvider::from_protocol(
            ...
            wf_llm::ProviderProtocol::OpenAiCompatible,
        )?;
        let config = provider.to_provider_config("");
        let client = Arc::new(ProviderClient::new(config, llm_provider));
        state
...
pub fn save_api_key(provider_id: &str, env_key: &str, api_key: &str) -> Result<()> {
    wf_persistence::save_configured_provider(provider_id, env_key, api_key)
}

pub fn save_selected_models(models: &[SelectedModel]) -> Result<()> {
    wf_persistence::save_selected_models(models)
}
...
pub async fn load_initial_state(state: &mut AppState) {
    ...
    let persisted = wf_persistence::load();
    ...
    // preserved, not overwritten.
    {
        ...
        for agent in pool.agents_mut() {
            let handle = wf_agent::sandbox::SandboxHandle::new(&agent.id)
    ...
    if let Some(mut session) = wf_persistence::load_session() {
...
pub fn save_custom_provider(
    ...
    if let Err(e) = wf_persistence::save_custom_provider(&provider) {
    ...
    if !key.is_empty() {
        ...
        let _ = wf_persistence::save_configured_provider(&custom_id, &env_key, key);
    } else if !state.configured_providers.contains(&custom_id) {
        state.configured_providers.push(custom_id.clone());
        let _ = wf_persistence::save_configured_provider(&custom_id, &env_key, "");
    }
...
pub fn merge_custom_providers(state: &mut AppState) {
    let custom_providers = wf_persistence::load_custom_providers();
