...
//! Business logic delegated to [`crate::controller`] and [`commands`].
    ...
    fn handle_input_submit(&self, state: &mut AppState) -> bool {
        ...
        let (provider, model_id, system_prompt) = {
            ...
            if agent_id.is_some() && was_unset {
                ...
                let server = wf_tools::ToolServer::new();
                let server = wf_tools::builtin::register_tools(server);
                let server = wf_tools::agent::register_tools(server, pool.clone(), tx, aid);
                let memo_deps = wf_tools::memo::MemoToolDeps::new(pool, aid);
                let server = wf_tools::memo::register_memo_tools(server, memo_deps);
