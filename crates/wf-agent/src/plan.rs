...
impl Plan {
    ...
    pub fn into_graph(self) -> GraphPlan {
        let mut graph = wf_core::task_graph::TaskGraph::new();
        ...
        for task in &self.tasks {
            if let Some(child_id) = graph.spawn_child(root_id, &task.description) {
                ...
                match task.status {
                    crate::plan::TaskStatus::Pending => {
                        // stays Created — resolved by ready_tasks()
                    }
                    crate::plan::TaskStatus::Running => {
                        let _ = graph.mark_ready(child_id);
                    }
                    crate::plan::TaskStatus::Completed => {
                        let _ = graph.mark_ready(child_id);
                        let _ = graph.mark_complete(child_id);
                    }
                    crate::plan::TaskStatus::Failed => {
...
pub struct GraphPlan {
    /// The underlying DAG.
    pub graph: wf_core::task_graph::TaskGraph,
    /// The root task node ID.
    pub root_id: wf_core::TaskId,
...
impl GraphPlan {
    ...
    pub fn spawn_subtask(&mut self, goal: &str) -> Option<wf_core::TaskId> {
    ...
    pub fn spawn_child_of(
        &mut self,
        parent_id: wf_core::TaskId,
        goal: &str,
    ) -> Option<wf_core::TaskId> {
    ...
    pub fn leaf_tasks(&self) -> Vec<wf_core::TaskId> {
    ...
    pub fn summary(&self) -> String {
        ...
        let done = counts
            .get(&wf_core::task_graph::TaskStatus::Completed)
            .copied()
            .unwrap_or(0);
        format!(
            "[{}/{}] {} | {}",
...
impl PlanGraphRegistry {
    ...
    pub fn find_by_task(&self, task_id: wf_core::TaskId) -> Vec<&str> {
        self.plans
            .iter()
