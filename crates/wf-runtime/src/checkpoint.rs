...
pub struct RuntimeSnapshot {
    ...
    pub role_memos: std::collections::HashMap<String, Vec<wf_agent::MemoEntry>>,
...
impl Checkpoint {
    ...
    pub fn save_pool(&self, pool: &AgentPool) -> Result<()> {
        ...
        wf_persistence::write_binary(&self.pool_path, &bytes)?;
        Ok(())
    }
    ...
    pub fn save_graph(&self, graph: &TaskGraph) -> Result<()> {
        ...
        wf_persistence::write_binary(&self.graph_path, &bytes)?;
        Ok(())
    }
    ...
    pub fn write_snapshot(&self, pool_bytes: &[u8], graph_bytes: &[u8]) -> Result<()> {
        wf_persistence::write_binary(&self.pool_path, pool_bytes)?;
        wf_persistence::write_binary(&self.graph_path, graph_bytes)?;
        Ok(())
    }
        ...
        for agent in &snapshot.agents {
            ...
            if matches!(
                ...
                wf_agent::AgentStatus::Planning
                    | wf_agent::AgentStatus::AwaitingChildren
                    | wf_agent::AgentStatus::Aggregating
            ) {
                restored.status = wf_agent::AgentStatus::Idle;
            }
...
pub async fn restore_checkpoint(
    agent_pool: &tokio::sync::RwLock<wf_agent::AgentPool>,
    task_graph: &std::sync::Mutex<wf_core::task_graph::TaskGraph>,
) -> bool {
    ...
    // Restore task graph.
    {
        ...
        *g = snapshot.task_graph;
        let reset_ids: Vec<wf_core::TaskId> = g
            .all_nodes()
            .filter(|n| {
                matches!(
                    n.status,
                    wf_core::task_graph::TaskStatus::Running
                        | wf_core::task_graph::TaskStatus::Dispatching
                )
            })
            .map(|n| n.id)
            .collect();
...
mod tests {
    ...
    fn test_save_and_load_graph() {
        ...
        assert_eq!(
            ...
            wf_core::task_graph::TaskStatus::Ready
        );
    }
