            ...
            match event {
                ...
                wf_llm::ToolEvent::ToolCall { name, args, .. } => {
                    ...
                    if let Ok(mut pool) = agent_pool.try_write() {
                        ...
                        // Record tool trace
                        if let Some(agent) = pool.get_agent_mut(&agent_id) {
                            agent.tool_trace.push_back(wf_agent::ToolCallRecord {
                                name: tool_name,
                                args_preview,
                                status: wf_agent::ToolStatus::Success,
                                error_message: None,
                            });
                            if agent.tool_trace.len() > wf_agent::MAX_TOOL_TRACE {
                                agent.tool_trace.pop_front();
                            }
                        }
                    }
                }
                wf_llm::ToolEvent::TokenUsage {
                    input,
                    output,
                    cached_input,
                ...
                wf_llm::ToolEvent::AgentStart
                | wf_llm::ToolEvent::AgentEnd
                | wf_llm::ToolEvent::TurnStart
                | wf_llm::ToolEvent::TurnEnd
                | wf_llm::ToolEvent::MessageStart
                | wf_llm::ToolEvent::MessageEnd => {}
                wf_llm::ToolEvent::Done { reason } => {
                    done_received = true;
...
mod tests {
    ...
    async fn test_tool_error_detection_marks_trace() {
        ...
        assert!(
            agent
                .tool_trace
                .iter()
                .any(|t| t.status == wf_agent::ToolStatus::Error),
    ...
    async fn test_no_false_positive_error_detection() {
        ...
        assert!(
            agent
                .tool_trace
                .iter()
                .all(|t| t.status == wf_agent::ToolStatus::Success),
    ...
    async fn test_tool_call_trace_overflow() {
        let (pool, aid) = make_pool_and_agent();
        let max = wf_agent::MAX_TOOL_TRACE;
