...
// RoleTemplate is re-exported from wf-core, no local serde needed
...
impl Default for AgentRuntimeConfig {
    fn default() -> Self {
        Self {
            max_concurrent_agents: wf_core::DEFAULT_MAX_AGENTS,
            admission_timeout_ms: wf_core::DEFAULT_ADMISSION_TIMEOUT_MS,
            max_depth: wf_core::DEFAULT_MAX_DEPTH,
            initial_budget: wf_core::DEFAULT_RUNTIME_BUDGET,
            l1_confidence_threshold: wf_core::DEFAULT_L1_CONFIDENCE,
            semantic_conflict_threshold: wf_core::DEFAULT_SEMANTIC_THRESHOLD,
            suspend_timeout_ms: wf_core::DEFAULT_SUSPEND_TIMEOUT_MS,
            bedrock_path: None,
...
/// Re-exported from wf-core to avoid circular dependencies.
pub use wf_core::RoleTemplate;
...
mod tests {
    ...
    fn test_role_template_with_embedding() {
        let mut emb = [0.0f32; wf_core::EMBEDDING_DIM];
        emb[0] = 0.5;
        emb[42] = -0.3;
        emb[383] = 0.9;
        let t = RoleTemplate {
        ...
        assert_eq!(e.len(), wf_core::EMBEDDING_DIM);
    }
    ...
    fn test_opt_big_array_serialize_none() {
        let arr: Option<[f32; wf_core::EMBEDDING_DIM]> = None;
        let mut buf = Vec::new();
        let mut ser = serde_json::Serializer::new(&mut buf);
        wf_core::types::opt_big_array_384::serialize(&arr, &mut ser).unwrap();
        let json = String::from_utf8(buf).unwrap();
    ...
    fn test_opt_big_array_deserialize_wrong_length() {
        let json = "[0.5, 0.5]";
        let mut de = serde_json::Deserializer::from_str(json);
        let result = wf_core::types::opt_big_array_384::deserialize(&mut de);
        assert!(result.is_err());
    }
}
