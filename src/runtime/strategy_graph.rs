...
impl StrategyGraph {
    ...
    pub fn select_strategy(
        ...
        let candidates: Vec<&StrategyNode> = self
            .strategies
            .values()
        ...
        // Exploitation: pick the best candidate by momentum-adjusted score.
        let mut scored: Vec<(StrategyId, f32)> = candidates
            ...
            .filter_map(|s| {
                let base_score = self
                    ...
                    .map(|e| if e.strategy_a == s.id { e.win_rate } else { 1.0 - e.win_rate })
    ...
    pub fn run_competition_cycle(&self) -> Vec<CompetitionAction> {
        Vec::new()
    }
}
