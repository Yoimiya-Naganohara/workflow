...
impl Default for DefaultEscalationPolicy {
    fn default() -> Self {
        Self { max_consecutive_failures: 3, latency_threshold_ms: 30_000 }
    }
}
...
mod tests {
    ...
    fn make_outcome(success: bool, latency_ms: u64) -> TaskOutcome {
        TaskOutcome { task_id: [0u8; 16], agent_id: None, success, role: "developer".into(), latency_ms, tokens_input: 100, tokens_output: 50 }
    }
    ...
    fn test_escalates_after_three_consecutive_failures() {
        let policy = DefaultEscalationPolicy::default();
        let task = TaskNode::new([0u8; 16], "test");
        let outcomes = vec![make_outcome(true, 100), make_outcome(false, 500), make_outcome(false, 200), make_outcome(false, 300)];
        let reason = policy.should_escalate(&task, &outcomes);
        assert!(reason.is_some());
        assert!(matches!(reason, Some(EscalationReason::RepeatedFailure { count: 3, .. })));
    }
    ...
    fn test_two_failures_not_enough() {
        let policy = DefaultEscalationPolicy::default();
        let task = TaskNode::new([0u8; 16], "test");
        let outcomes = vec![make_outcome(true, 100), make_outcome(false, 200), make_outcome(false, 300)];
        assert!(policy.should_escalate(&task, &outcomes).is_none());
    }

    #[test]
    fn test_escalates_on_high_latency() {
        let policy = DefaultEscalationPolicy { max_consecutive_failures: 3, latency_threshold_ms: 100 };
        let task = TaskNode::new([0u8; 16], "test");
        let outcomes = vec![make_outcome(true, 500)];
        assert!(matches!(policy.should_escalate(&task, &outcomes), Some(EscalationReason::HumanRequired { .. })));
    }
}
