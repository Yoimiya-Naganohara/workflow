...
impl TaskScheduler {
    ...
    pub async fn dispatch(&self) {
        ...
        for (task_id, task_goal, task_role) in ready {
            ...
            let selected_strategy: Option<(Option<StrategyId>, u64)> = self.strategy_graph.as_ref().map(|sg| {
            ...
            let decision_success = matches!(decision_is_approved, DispatchDecision::Approved { .. });
            match decision_is_approved {
                ...
                DispatchDecision::Escalate { target_role, reason } => {
                    ...
                    let rt = self.runtime.read().await;
                    let mut g = rt.task_graph.lock().unwrap_or_else(|e| e.into_inner());
                    if let Some(node) = g.get_mut(&task_id) { node.metadata.insert("escalated_to".into(), target_role); node.metadata.insert("escalated_reason".into(), reason); }
