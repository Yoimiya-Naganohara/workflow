...
pub struct UiState {
    ...
    pub current_plan: Option<wf_agent::plan::Plan>,
    ...
    pub tree_agent_ids: Vec<wf_core::AgentId>,
...
mod tests {
    ...
    use wf_runtime::runtime::{AgentRuntime, config::AgentRuntimeConfig};

    use wf_runtime::test_utils::MockEmbed;
    ...
    fn test_system_prompt_does_not_contain_goal() {
        ...
        assert!(prompt.contains(wf_core::MEMO_INSTRUCTIONS));
    }
    ...
    async fn test_full_chat_pipeline_with_runtime() {
        let embed: Arc<dyn wf_llm::EmbeddingService> = Arc::new(MockEmbed);
