...
pub struct Agent {
    ...
    pub task_id: Option<wf_core::TaskId>,
...
pub struct AgentPool {
    agents: Vec<Agent>,
    #[serde(skip)]
    pub(crate) provider: Option<Arc<wf_llm::LlmProvider>>,
    #[serde(skip)]
    ...
    /// Prefer `write_role_memo` / `delete_role_memo` over direct mutation
    /// to ensure dedup and persistence consistency.
    ...
    pub reasoning_options: Vec<wf_core::ReasoningOption>,
...
impl AgentPool {
    pub fn new() -> Self {
        Self {
            ...
            ttl_secs: wf_core::SECONDS_PER_HOUR,
            max_agents: wf_core::DEFAULT_MAX_AGENTS,
    ...
    pub fn set_provider(&mut self, provider: wf_llm::LlmProvider) {
    ...
    pub fn mark_execution_start(&mut self, agent_id: &AgentId) {
        let metrics = self.agent_metrics.entry(*agent_id).or_default();
        metrics.started_at = crate::now_secs();
    }
    ...
    pub fn mark_execution_complete(&mut self, agent_id: &AgentId) {
        if let Some(metrics) = self.agent_metrics.get_mut(agent_id) {
            metrics.completed_at = crate::now_secs();
        }
    }
